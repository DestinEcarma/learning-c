...
int dateDifference(MyDate a, MyDate b) {
	int daysA =
		a.day + (a.month - 1) * getDaysOfMonth(a.month, a.year) + a.year * 365;
	int daysB =
		b.day + (b.month - 1) * getDaysOfMonth(b.month, b.year) + b.year * 365;

	if (isLeapYear(a.year) && a.month < 3)
		daysA--;
	if (isLeapYear(b.year) && b.month > 2)
		daysB++;
...
void printStaticList(ProductListStatic *list) {
	...
	for (int i = 0; i < list->count; i++) {
		...
		printf("%d\t%s\t%02d-%02d-%04d\t%.2f\n", temp.id, temp.name,
			   temp.exDate.day, temp.exDate.month, temp.exDate.year,
			   temp.price);
	}
...
ProductListDynamic *filter(ProductListStatic *list, MyDate expiry) {
	...
	for (int i = 0; i < list->count; i++) {
		if (dateDifference(list->prods[i].exDate, expiry) < 0)
			continue;
		...
		for (; j < filtered->count; j++) {
			if (dateDifference(temp.exDate, filtered->prods[j].exDate) > 0)
				break;
		}

		memmove(filtered->prods + j + 1, filtered->prods + j,
				(filtered->count - j) * sizeof(Product));
		memmove(list->prods + i, list->prods + i + 1,
				(list->count - i - 1) * sizeof(Product));
